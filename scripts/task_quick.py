...
TEMPLATES = {
    ...
    "design": {
        ...
        "context": "Design task dispatched via CENTRAL. Output is a document or structured spec, not running code. Follow the LLD Worker Guidelines at docs/lld_worker_guidelines.md — especially §1.3 (forced design decisions) and §3 (trace before fixing on revisions).",
        "scope": "Research options, evaluate trade-offs, and produce a written recommendation. Do not implement. Flag open questions explicitly. If scope exceeds 5 major subsystems, propose a split.",
        "deliverables": "Design doc or decision record committed to the repo under docs/. Downstream task stubs if applicable.",
        "acceptance": "Design is complete, coherent, and actionable. Open questions are listed with owners. Cross-references are internally consistent. Review tool has been run (see guidelines §4).",
        "testing": "Peer-review checklist: objective is clear, scope is bounded, trade-offs are explicit, forced design decisions are answered, follow-on tasks are identified.",
